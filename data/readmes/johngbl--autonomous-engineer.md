...
> An enterprise, self-sufficient Meta-Skill that transforms coding agents (Claude Code, Roo Code, OpenHands, Codex App, Cursor, Aider, Zed) into Staff/Principal-level autonomous software engineers.
...
├── .gitignore
├── setup.sh                  <- One-command bootstrap for target projects
│
├── .github/
│   └── workflows/
...
└── evals/                    <- EDD: validates the skill's own behavior
    ...
    └── cases/                <- 5 foundational behavioral test cases
```
...
### 3. Roo Code / Cline (VS Code Extension)
```bash
cp SKILL.md .clinerules
cp -r registry/ .claude/registry/
cp -r templates/ .claude/templates/
```

### 4. Cursor / Windsurf
```bash
cp SKILL.md .cursorrules   # For Cursor
cp SKILL.md .windsurfrules # For Windsurf
cp -r registry/ .claude/registry/
cp -r templates/ .claude/templates/
```
...
Instead of manually copying files, use the bootstrap script:

```bash
# From the autonomous-engineer repo root:
./setup.sh /path/to/my-project

# Or from within the target project:
/path/to/autonomous-engineer/setup.sh .
```
