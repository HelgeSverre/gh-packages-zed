...
1. `lsp.taskpaper-ls.binary.path` from your Zed settings, if set;
2. a `taskpaper-ls` on the worktree's PATH;
3. otherwise it downloads the binary for your platform from this repo's
   [GitHub releases](https://github.com/ickc/zed-taskpaper/releases) and
   caches it. Prebuilt binaries cover macOS, Linux, and Windows on both
   x86-64 and arm64.

Everything except the language server features works without that download.

To run from a checkout instead — `zed: extensions` → `Install Dev
Extension` → select the clone. Build the server with `pixi run build-lsp`
...
See [MAINTAINER.md](MAINTAINER.md). [MIT](LICENSE) licensed.
